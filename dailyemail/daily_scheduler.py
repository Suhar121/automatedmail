# daily_scheduler.py - Updated for Render
import smtplib
import ssl
...
class DailyTopicGenerator:
    def __init__(self):
        ...
        if api_key:
            ...
            self.model = None
            logger.warning("No Gemini API key found, using fallback topics")
    ...
    def generate_topic_description(self, topic):
        ...
        try:
            ...
            logger.info("Generated topic description")
            return description
    ...
    def send_daily_topic_email(self):
        ...
        try:
            ...
            if success:
                logger.info(f"✅ Daily topic email sent: {topic}")
                return True
            else:
                logger.error("❌ Failed to send daily topic email")
                return False
...
class Scheduler:
    ...
    def schedule_daily_email(self):
        ...
        schedule.every().day.at("13:00").do(self._send_scheduled_email)
        
        logger.info("🕐 Daily topic scheduler started...")
        logger.info("📧 Emails scheduled for 1:00 PM daily")
        logger.info(f"⏰ Next run: {schedule.next_run()}")
    
    def _send_scheduled_email(self):
        """Send the scheduled daily email"""
        logger.info(f"🕐 Sending scheduled email at {datetime.now()}")
    ...
    def start_scheduler(self):
        ...
        logger.info("📧 Daily topics will be sent at 1:00 PM")
...
if __name__ == "__main__":
    logger.info("🚀 Starting Daily Topic Scheduler on Render...")
    ...
    # Start the scheduler
    scheduler, scheduler_thread = start_daily_scheduler()
